...
public class ApiTest : MonoBehaviour, IMapProgressManager
{
    ...
    public void OnGUI()
    {
        ...
        {
            DrawInputParameters();
            /* if (GUILayout.Button("Complete all  levels", GUILayout.Width(250), GUILayout.Height(40)))
             {
                 LevelManager.THIS.PassedLevels = 600;
                 PlayerPrefs.SetInt("PassedLevels", LevelManager.THIS.PassedLevels);
                 Debug.Log(LevelManager.THIS.PassedLevels);
                 for (int i = 1; i < 600; i++)
                 {
            ...
            if (GUILayout.Button("Complete level", GUILayout.Width(400), GUILayout.Height(80)))
            {
                //LevelManager.THIS.currentLevel = _levelNumber;
                //LevelManager.THIS.PassedLevels = LevelManager.THIS.currentLevel;
                //Debug.Log("Passed: " + LevelManager.THIS.PassedLevels);
                //PlayerPrefs.SetInt("PassedLevels", LevelManager.THIS.PassedLevels);
                //PlayerPrefs.SetInt("PassedLevelsOfline", LevelManager.THIS.PassedLevels);

                if (LevelsMap.IsStarsEnabled())
                    LevelsMap.CompleteLevel(_levelNumber, _starsCount);
                else
                    LevelsMap.CompleteLevel(_levelNumber);
                //  LevelManager.THIS.SendToServer(true);

                _levelNumber++;
               // Debug.Log("Currunt level: " + LevelManager.THIS.currentLevel);
            }

            if (GUILayout.Button("Go to level", GUILayout.Width(400), GUILayout.Height(80)))
            {
                LevelsMap.GoToLevel(_levelNumber);
            }

            if (GUILayout.Button("Is level locked", GUILayout.Width(400), GUILayout.Height(80)))
            {
            ...
            if (GUILayout.Button("Clear all progress", GUILayout.Width(400), GUILayout.Height(80)))
            {
                // PlayerPrefs.SetInt("PassedLevels", PlayerPrefs.GetInt("PassedLevels") - 100);
    ...
    private void DrawToggleShowButton()
    {
        if (!_isShow)
        {
            if (GUILayout.Button("Show API tests", GUILayout.Width(400), GUILayout.Height(80)))
            {
                _isShow = true;
               // LevelManager.THIS.currentLevel = _levelNumber;
        ...
        if (_isShow)
        {
            if (GUILayout.Button("Hide API tests", GUILayout.Width(400), GUILayout.Height(80)))
            {
                _isShow = false;
            }
        }
    ...
    private void DrawInputParameters()
    {
        ...
        string strLevelNumber = GUILayout.TextField(_levelNumber.ToString(), 10, GUILayout.Width(80), GUILayout.Height(80));
        int.TryParse(strLevelNumber, out _levelNumber);

        if (LevelsMap.IsStarsEnabled())
        {
            GUILayout.Label("Stars count:");
            string strStarsCount = GUILayout.TextField(_starsCount.ToString(), 10, GUILayout.Width(80), GUILayout.Height(80));

...
public class LevelMakerEditor : EditorWindow
{
    ...
    void OnFocus()
    {
        ...
        if (EditorSceneManager.GetActiveScene().name == "game" || EditorSceneManager.GetActiveScene().name == "game 1")
        {
...
//            GooglePlayServices.PlayServicesResolver.MenuResolve();//2.1.2
    ...
    void GUILevelSelector()
    {
        ...
        if (GUILayout.Button(">>", new GUILayoutOption[] { GUILayout.Width(50) }) || Input.GetKeyDown(KeyCode.RightArrow))
        {
            NextLevel();
        }
    ...
    void GUIColorLimit()
    {
        ...
        GUILayout.Label("Card limit:", EditorStyles.label, new GUILayoutOption[] { GUILayout.Width(100) });
        colorLimit = (int)GUILayout.HorizontalSlider(colorLimit, 3, 80, new GUILayoutOption[] { GUILayout.Width(100) });
    ...
    public void SaveMap(string fileName)
    {
        ...
        for (int row = 0; row < maxRows; row++)
        {
            for (int col = 0; col < maxCols; col++)
            {
                saveString += (int)levelSquares[row * maxCols + col].block + "";
        ...
        foreach (string line in lines)
        {
            ...
            else if (line.StartsWith("COLLECT COUNT "))
            {
                ...
                if (target == Target.AnaMasry)
                {
                    ...
                    {
                        Debug.Log("sssssss");
                        AnaCount[i] = int.Parse(blocksNumbers[i]);
                    }
                }
                else
                {
            ...
            else if (line.StartsWith("COLLECT ITEMS "))
            {
                ...
                for (int i = 0; i < blocksNumbers.Length; i++)
                {
                    ...
                    {
                        Debug.Log("Anamasr");
                        anamasry[i] = (ANAMASRY)int.Parse(blocksNumbers[i]);
                    }
                }
            }
            else

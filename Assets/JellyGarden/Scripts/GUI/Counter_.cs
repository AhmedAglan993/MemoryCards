...
public class Counter_ : MonoBehaviour
{
    ...
    void Update()
    {
        if (name == "InGameScore")
        {
            txt.text = "Score: " + LocalGameManager.Instance.Score;
        }
        if (name == "Score")
        {
            txt.text = "" + LocalGameManager.Instance.Score;
        }
        if (name == "Limit")
        {
            if (LocalGameManager.Instance.limitType == LocalGameManager.LimitType.moves)
            {
                txt.text = "Try to find the matches in " + "\n" + LocalGameManager.Instance.Limit + " moves";
            }
            else
            {
                txt.text = "Try to find the matches \n in " + Timer.Instance.UpdateTimer(LocalGameManager.Instance.Limit) + " sec";
            }
        }
        if (name == "Coins")
        {
            txt.text = "Coins: " + PlayerPrefs.GetInt("Coins");
        }
        if (name == "Trip count")
        {
            txt.text = "" + PlayerPrefs.GetInt("Coins");
        }
        if (name == "BestScore")
        {
        ...
        if (name == "Level")
        {
            txt.text = "" + OpenLevel.Instance.curruntLevel;
        ...
        if (name == "TargetDescription1")
        {
            ...
            else if (LevelManager.THIS.target == Target.AnaMasry)
                txt.text = "Collect ANA MASRY Word";

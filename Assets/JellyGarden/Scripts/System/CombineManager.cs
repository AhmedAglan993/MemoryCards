...
public class CombineManager
{
    ...
    bool CheckColor(Item item, Item nextItem)
    {
        if (nextItem)
        {
            if (nextItem.color == item.color && nextItem.currentType != ItemsTypes.BOMB && nextItem.currentType != ItemsTypes.INGREDIENT && nextItem.currentType != ItemsTypes.ANAMASRY)//2.0
                return true;
        }
        return false;
    }
